...
plugins/
  mcp-issue-loop/           # plugin: autonomous ai-ready issue loop
...
| Plugin | What it does |
|--------|--------------|
| [`mcp-issue-loop`](plugins/mcp-issue-loop/) | Ships **`issue-build-loop`** — works the open `ai-ready` issues in any repo, one worktree + subagent per issue (max 3 parallel), each driven to a CI-green PR, then iterated against review feedback until you approve and it merges. It detects the repo's shape and picks the matching build playbook: the full Umbraco MCP skills workflow for MCP repos, or a lightweight skill/plugin/script/markdown playbook for repos *without* the toolchain (this ops repo, docs/plugin repos — **not** `Umbraco-MCP-Base`, which is a full MCP repo and gets the MCP playbook); the latter is the converter for the `loop-improvement` issues triage files here. Also ships **`issue-discuss-loop`** — the step *before* `ai-ready`: label an issue `ai-discussing` and it writes a stub issue properly, asks questions when it can't yet, or critiques a written issue antagonistically, one comment per fire (your reply fires the next round) until you both agree on the smallest change that solves the problem. Comments address the loop by default; start one with `//` to ask a colleague instead and it stays out. Discussion only — no code, no PR. Repo-agnostic; runs locally or as an event-fired cloud routine. |
| [`self-learning`](plugins/self-learning/) | The self-learning loop, shared by every loop in this repo — not specific to `issue-build-loop` or any other. Capture hooks (`SubagentStop`/`SessionEnd`) append `proto-learning` rows to a shared Slack canvas whenever a loop run has something worth improving, matching on this ecosystem's shared conventions (`github-ops`, `/goal`) rather than naming individual loops — a new loop needs no integration to get captured. Bundles `triage-learnings` (Loop B), which periodically routes each canvas row to the repo that owns it — a tracked issue on the specific MCP repo it affects (domain-specific learnings only), a gated PR to the shared `umbraco-mcp-skills` (`Umbraco-MCP-Base`) for generalizable ones, or a `loop-improvement` issue here for the loop itself. Loop B files issues to owning repos and only drafts PRs for the shared tooling. Install alongside any loop plugin to get capture for free. |
| [`release-flow`](plugins/release-flow/) | Branching, merge, release, and dev-sync workflow skills for any repo — detects gitflow (`dev` + `main`) vs main-only and follows the matching conventions for branch naming, squash vs merge-commit, cutting a release, tagging, and syncing back to `dev`. Bundles `release-and-branching`, `sync-dev`, and **`auto-release-loop`** — an issue+label-triggered, CI-gated release loop: label an issue titled `release <version>` with `auto-releasing`, and it cuts the branch, bumps + changelogs, drives CI green, then (CI is the gate — no approval pause) merges, tags, publishes the GitHub Release, and syncs `main`→`dev`, with Claude push notifications at start + completion. Uses `/goal`. |
| [`merge-flow`](plugins/merge-flow/) | Guardrail loop that merges PRs labelled `auto-merging` — but only once they're approved, CI-green (polled, never `--auto`), conflict-free, and on the right base. Replaces error-prone manual merges; drives each to a clean merge (branch deleted) or flags the blocker. Uses `/goal`. Repo-agnostic; local or scheduled cloud routine. |
...
> **Note:** `issue-build-loop` drives local worktrees, builds, and integration tests,
