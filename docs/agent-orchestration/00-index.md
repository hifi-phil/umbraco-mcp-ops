...
| [`../../worker/README.md`](../../worker/README.md) | *(new)* The real receiver | A Cloudflare Worker + Durable Object exist as code now, importing `graph/` directly — 40 unit tests, clean type-check, and a real local smoke test via `wrangler dev --local` against a stub API server (real routing, real `translate()`/`reduce()`, a real D1 log row). Not deployed anywhere — no live Cloudflare account access for this repo. Built as Phase 4's enforcement mechanism (real writes, no toggle), not Phase 3's — see [07-build-phases.md](07-build-phases.md)'s status notes and the shadow-mode-toggle gap in [08-open-questions.md](08-open-questions.md) |
