...
| [`../../worker/README.md`](../../worker/README.md) | The real receiver | A Cloudflare Worker + Durable Object exist as code now, importing `graph/` directly — 40 unit tests, clean type-check. Converged into a **black-box harness**: everything real (Worker, DO, D1, the reducer, the mock's webhook emission) except the five loops, each black-boxed the way it's actually observed — a real `@anthropic-ai/claude-agent-sdk` session vs. verbatim skill text for the two loops that self-report an outcome comment (`issue-build-loop`'s Step 3, `auto-release-loop`'s Step 2.5/Step 4 — the full `agent-outcomes` catalog), a direct synthetic webhook for the two loops whose real signal is a native GitHub event (`rework-loop`'s push, `merge-flow`'s merge — no agent, more faithful, not a shortcut). Run for real end to end, all four self-reported outcomes and both native signals landed correctly, and it surfaced two real findings: the outcome artifact's reducer rule only fires for `release_published` (the one case that doesn't clear its label before commenting — the other three always drop as "no matching rule", see [08-open-questions.md](08-open-questions.md)), and `issue-build-loop`'s live skill text was still on the pre-rename label spelling while `graph/` already used the new one (fixed across all 14 referencing files; see [10-label-rename.md](10-label-rename.md) for the coordination hazard that fix now creates). Not deployed anywhere — no live Cloudflare account access for this repo. Built as Phase 4's enforcement mechanism (real writes, no toggle), not Phase 3's — see [07-build-phases.md](07-build-phases.md)'s status notes and the shadow-mode-toggle gap in [08-open-questions.md](08-open-questions.md) |
