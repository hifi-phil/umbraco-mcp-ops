...
description: >-
  ...
  then post one Slack digest, flagging any repo whose "Automatically delete head branches"
  setting is off. Informational and read-only — it never deletes anything. Removing branches
  is a separate, deliberate step: the `/clean-branches` command, which is local-only. GitHub
  work goes through the required `github-ops` skill. Trigger on "what's the state of the
  branches", "run branch housekeeping", "which branches can I delete", "branch report".
---
...
gh absent   (cloud routine) →  classify via mcp__github__*  (MCP reference)
```
...
**Cloud path** — no `gh`: work through `classification.md` using the `mcp__github__*` tools.
Same rules, same categories, same digest.
...
(*Get file contents*, `hifi-phil/umbraco-mcp-ops`, path
...
There is no step 3 — don't offer to clean up. If the digest shows removable branches, name
`/clean-branches` per [`references/report.md`](references/report.md#naming-the-cleanup-without-doing-it)
and stop.
...
- **Read-only, always.** Never delete a branch, and never run `reap.sh` from this skill — not
  ...
  split.
- **The reuse guard is the least obvious rule here**
  ([`references/classification.md`](references/classification.md)) — the one that protects
  live release lines. Never report a reused branch as safe.
- **The two paths must agree.** Changing classification behavior means changing
  `classification.md` and `sweep.sh` together — see classification.md's own note on why a
  divergence is worse than either single answer.
