...
2. If it is, decide whether something **non-obvious** happened, per the schema's
   when-to-file categories. Apply the schema's **signal-not-noise** bar: if the run
   was clean and by-the-book, output `{"file":false}`. When in doubt, do **not**
   file. One learning per finding — pick the single most valuable one if several
   exist.
